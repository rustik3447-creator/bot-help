...
# --- Налаштування ---
# Вставте ваш дійсний токен від BotFather:
...
def run_flask():
  # Отримуємо порт із системних змінних або використовуємо 8080 за замовчуванням
...
def get_main_menu():
  ...
  markup.add('🚗 ст. 122 ПДР', '⚖️ ст. 173-2 Домашнє насильство')
...
def art122_parking_info(message):
  text = (
      ...
      '• **у втором ряді** (поруч з іншим транспортним засобом, який стоїть'
      ' біля краю проїзною частини (на узбіччі)), чим порушив п.15.4 ПДР.'
  )
  bot.send_message(message.chat.id, text, parse_mode='Markdown')
