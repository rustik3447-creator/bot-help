...
@bot.message_handler(
    func=lambda message: bool(message.text) and ('схожого на зброю' in message.text.lower() or 'предмета схожого' in message.text.lower())
)
def weapon_like_algorithm_info(message):
    text = (
        "🗡 <b>Алгоритм дій інспектора «Служби освітньої безпеки» у разі виявлення у дитини предмета, схожого на зброю</b>\n\n"
        ...
        "<b>Якщо дитина не реагує:</b>\n"
        "• Заняти безпечне місце та слідкувати щоб сторонні люди не наближались до місця події.\n"
        ...
        "4. Викликати екстренні служби (за необхідністю)\n"
    ...
    markup = types.InlineKeyboardMarkup()
    btn_report = types.InlineKeyboardButton("📋 Приклад рапорту", callback_data="weapon_report_example")
    markup.add(btn_report)
    
    bot.send_message(message.chat.id, text, parse_mode='HTML', reply_markup=markup)

@bot.callback_query_handler(func=lambda call: call.data == "weapon_report_example")
def handle_weapon_report_example(call):
    bot.answer_callback_query(call.id)
    text = (
        "📋 <b>ПРИКЛАД РАПОРТУ (ВИЯВЛЕННЯ ПРЕДМЕТА, СХОЖОГО НА ЗБРОЮ)</b>\n\n"
        "<b>СОБ 107</b>\n"
        "Вул. Академіка Барабашова, 38Б.\n\n"
        "19.08.2026 близько 10 год 15 хв під час несення служби в ЗЗСО до інспектора СОБ звернувся класний керівник, "
        "який повідомив про наявність у учня предмета, схожого на зброю (пістолет). Інспектором СОБ було негайно вжито "
        "заходів щодо забезпечення безпеки учнів, огороджено місце події, проінформовано адміністрацію закладу та викликано СОГ.\n\n"
        "На місці працювали: СОГ ХРУП 1, екіпаж патрульної поліції.\n\n"
        "Предмет вилучено співробітниками СОГ для проведення експертизи. Постраждалих немає.\n\n"
        "<b>ЄО 15234 ХРУП 1.</b>"
    )
    bot.send_message(call.message.chat.id, text, parse_mode="HTML")
...
def doc_685_1013_info(message):
    ...
    markup.add(
        types.InlineKeyboardButton(
            ...
            url='https://zakon.rada.gov.ua/laws/show/z1123-18#Text',
        )
    )
    bot.send_message(
        message.chat.id,
...
def doc_1376_info(message):
    ...
    markup.add(
        types.InlineKeyboardButton(
            '🔗 Відкрити Наказ № 1376',
            url='https://zakon.rada.gov.ua/laws/show/z1496-18#Text',
        )
    )
    bot.send_message(
        message.chat.id,
...
def about_bot_info(message):
    text = (
        'ℹ️ **Службовий помічник інспектора СОБ**\n\n'
        'Цей бот розроблено для оперативної допомоги інспекторам Служби'
        ' освітньої безпеки під час виконання службових обов’язків.\n\n'
        '**Основний функціонал:**\n'
        '• Швидкий доступ до фабул КУпАП (ст. 122, 127, 173, 173-2, 175-1, 178, 184, Булінг);\n'
        '• Алгоритми дій при виявленні ВНП, зброї, наркотичних речовин, домашньому насильстві та булінгу;\n'
        '• Нормативно-правова база (Закони, Постанови КМУ, Накази МВС/МОН);\n'
        '• Можливість екстреної відправки сигналу SOS керівництву та колегам;\n'
        '• Швидкий перехід до форм щоденної звітності.\n\n'
