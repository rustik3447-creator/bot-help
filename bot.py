...
def get_art184_menu():
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True, row_width=2)
    markup.add('🚸 ч. 1 ст. 184 КУпАП', '🚸 ч. 2 ст. 184 КУпАП')
    markup.add('🎒 ч. 3 ст. 184 КУпАП', '🚨 ч. 4 ст. 184 КУпАП')
    markup.add('🔙 Головне меню')
    return markup
...
def about_bot(message):
    text = (
        "ℹ️ <b>Служба освітньої безпеки (СОБ) — Робочий бот-помічник</b>\n\n"
        "Даний бот створений для швидкого доступу до фабул КУпАП, нормативно-правових актів, "
        "алгоритмів дій у надзвичайних ситуаціях та швидкої подачі звітів і SOS-сигналів."
    )
...
@bot.message_handler(
    func=lambda message: bool(message.text)
    and ('184' in message.text or 'Невиконання обов\'язків' in message.text)
    and not any(x in message.text for x in ['ч. 1', 'ч. 2', 'ч. 3', 'ч. 4'])
)
def art184_category_select(message):
    bot.send_message(
        message.chat.id,
        '🚸 **ст. 184 КУпАП — Невиконання батьками обов’язків щодо виховання дітей**\n\n'
        'Оберіть потрібну частину статті для перегляду фабули та переліку документів:',
        reply_markup=get_art184_menu(),
        parse_mode='Markdown'
    )

@bot.message_handler(
    func=lambda message: bool(message.text)
    and '184' in message.text
    and 'ч. 1' in message.text
)
def art184_part1_info(message):
    text = (
        '🚸 <b>ч. 1 ст. 184 КУпАП</b>\n\n'
        '📌 <b>Суть:</b> Ухилення батьків або осіб, які їх замінюють, від виконання передбачених законодавством обов’язків щодо забезпечення необхідних умов життя, навчання та виховання неповнолітніх дітей.\n\n'
        '📝 <b>Приклад фабули (малолітня дитина до 14 років вчинила правопорушення):</b>\n'
        '<i>«15.05.2025 о 12 год. 00 хв. за адресою м. Харків, вул. Садова, 15, гр. (ПІБ батька/матері), ухилився(-лась) від виконання передбачених ст. 150 Сімейного кодексу України батьківських обов’язків щодо виховання свого малолітнього сина (ПІБ дитини), 20.05.2015 р.н., внаслідок чого останній перебуваючи у приміщенні КЗ ХЛ №107 вчинив дрібне хуліганство (виражався нецензурною лайкою в бік однокласників), чим вчинив(-ла) правопорушення, передбачене ч. 1 ст. 184 КУпАП.»</i>\n\n'
        '📎 <b>ДО ПРОТОКОЛУ ДОДАЄТЬСЯ:</b>\n'
        '1. Рапорт поліцейського\n'
        '2. Пояснення батьків\n'
        '3. Пояснення дитини (у присутності психолога/педагога)\n'
        '4. Пояснення свідків / потерпілих\n'
        '5. Копія свідоцтва про народження дитини\n'
        '6. Копія паспорта та ІПН одного з батьків\n'
        '7. Відеозапис з боді-камери'
    )
    bot.send_message(message.chat.id, text, parse_mode='HTML')
...
def art184_part3_info(message):
    text = (
        '🎒 <b>ч. 3 ст. 184 КУпАП</b>\n\n'
        '📌 <b>Суть:</b> Вчинення неповнолітнім віком від 14 до 16 років діяння, що містить ознаки правопорушення, відповідальність за яке передбачена КУпАП (якщо дитина не досягла віку адмінвідповідальності - 16 років).\n\n'
        '📝 <b>Приклад фабули:</b>\n'
        '<i>«15.05.2025 о 11 год. 30 хв. за адресою м. Харків, вул. Світла, 2, гр. (ПІБ батька/матері), не здійснив(-ла) належного контролю за поведінкою свого сина (ПІБ дитини), 12.04.2010 р.н. (15 років), який вчинив правопорушення, передбачене ст. 173 КУпАП (виражався нецензурною лайкою у громадському місці). Оскільки неповнолітній не досяг віку адмінвідповідальності, відносно батька складено протокол за ч. 3 ст. 184 КУпАП.»</i>\n\n'
        '📎 <b>ДО ПРОТОКОЛУ ДОДАЄТЬСЯ:</b>\n'
        '1. Рапорт\n'
        '2. Пояснення дитини, батьків, свідків\n'
        '3. Копія свідоцтва про народження / паспорта дитини\n'
        '4. Копія паспорта батьків\n'
        '5. Відео з боді-камери'
    )
    bot.send_message(message.chat.id, text, parse_mode='HTML')
...
@bot.message_handler(
    func=lambda message: bool(message.text)
    and '184' in message.text
    and 'ч. 4' in message.text
)
def art184_part4_info(message):
    text = (
        '🚨 <b>ч. 4 ст. 184 КУпАП</b>\n\n'
        '📌 <b>Суть:</b> Вчинення неповнолітнім віком від 14 до 16 років діяння, що містить ознаки кримінального правопорушення, якщо він не досяг віку, з якого настає кримінальна відповідальність.\n\n'
        '📝 <b>Приклад фабули:</b>\n'
        '<i>«15.05.2025 о 14 год. 00 хв. гр. (ПІБ батька/матері) ухилився(-лась) від виконання батьківських обов’язків, внаслідок чого неповнолітній син (ПІБ), 10.10.2011 р.н., вчинив суспільно небезпечне діяння, що підпадає під ознаки ч.1 ст. 185 КК України (таємне викрадення чужого майна), не досягши віку кримінальної відповідальності. Чим вчинив(-ла) правопорушення, передбачене ч. 4 ст. 184 КУпАП.»</i>\n\n'
        '📎 <b>ДО ПРОТОКОЛУ ДОДАЄТЬСЯ:</b>\n'
        '1. Рапорт\n'
        '2. Матеріали з ВП / ЄО (витяг з ЄРДР або постанова про закриття кримінального провадження у зв’язку з недосягненням віку)\n'
        '3. Пояснення учасників\n'
        '4. Копії документов та відео з БК'
    )
    bot.send_message(message.chat.id, text, parse_mode='HTML')
...
@bot.message_handler(
    func=lambda message: bool(message.text)
    and ('173' in message.text or 'Дрібне хуліганство' in message.text)
...
def art173_info(message):
    text = (
        ...
        '<i>«15.05.2025 о 12 год. 30 хв. у м. Харків, вул. Садова, 15, гр. (ПІБ), перебуваючи у громадському місці, '
        'виражався нецензурною лайкою на адресу громадян, демонстративно чіплявся до перехожих, чим порушив громадський порядок і спокій громадян та вчинив правопорушення, передбачене ст. 173 КУпАП.»</i>\n\n'
        '📎 <b>ДО ПРОТОКОЛУ ДОДАЄТЬСЯ:</b>\n'
        '1. Рапорт поліцейського\n'
        '2. Пояснення свідків та потерпілих (за наявності)\n'
        '3. Пояснення правопорушника\n'
        '4. Відеозапис з боді-камери\n'
...
@bot.message_handler(
    func=lambda message: bool(message.text)
    and ('Домашнє насильство' in message.text or '173-2' in message.text)
)
...
@bot.message_handler(
    func=lambda message: bool(message.text) and any(doc in message.text for doc in [
        ...
        'Наказ № 1646', 'Наказ № 685/1013', 'Наказ № 1395', 'Наказ № 1376'
    ])
)
...
# --- НОВІ ОБРОБНИКИ ДЛЯ ОСТАННІХ ЧОПИРИХ АЛГОРИТМІВ ---

@bot.message_handler(
    func=lambda message: bool(message.text) and 'наркотичних речовин' in message.text
)
def drugs_algorithm_info(message):
    text = (
        "💊 <b>Дії у разі виявлення наркотичних речовин в учня:</b>\n\n"
        "1. <b>Обов'язково увімкнути боді-камеру.</b>\n"
        "2. <b>Припинити правопорушення</b> та забезпечити неможливість знищення або передачі речовин третім особам.\n"
        "3. <b>Негайно повідомити керівника ЗЗСО</b>, адміністрацію та викликати слідчо-оперативну групу (СОГ) через лінію 102.\n"
        "4. <b>Повідомити керівництво відділу СОБ та ювенальну превенцію (СЮП).</b>\n"
        "5. <b>Забезпечити присутність батьків/законних представників</b> (якщо неповнолітній) та практичного психолога чи педагога під час подальших дій.\n"
        "6. <b>Заборонено самостійно розкриватигого пакунки</b> або торкатися підозрілих речовин без засобів захисту.\n"
        "7. <b>Здійснити документування події</b>, зафіксувати свідків, відібрати первинні пояснення та передати матеріали прибулій СОГ."
    )
    bot.send_message(message.chat.id, text, parse_mode='HTML')

@bot.message_handler(
    func=lambda message: bool(message.text) and 'від 16 до 18 років' in message.text
)
def minor_16_18_algorithm_info(message):
    text = (
        "🧑‍🎓 <b>Дії у разі правопорушення особою від 16 до 18 років:</b>\n\n"
        "1. <b>Обов'язково увімкнути боді-камеру.</b>\n"
        "2. <b>Встановити особу правопорушника</b> за документами (паспорт, ID-картка).\n"
        "3. <b>Повідомити адміністрацію ЗЗСО та батьків (законних представників)</b> про вчинене правопорушення.\n"
        "4. <b>Провести опитування</b> та скласти адміністративний протокол безпосередньо на особу правопорушника (оскільки з 16 років настає загальна адміністративна відповідальність за КУпАП).\n"
        "5. <b>Додати до матеріалів:</b> рапорт, пояснення учасників та свідків, відео з боді-камери, копію паспорта правопорушника.\n"
        "6. <b>Здійснити реєстрацію події</b> (за потреби на лінію 102 або відповідно до внутрішніх інструкцій) та надіслати електронний рапорт."
    )
    bot.send_message(message.chat.id, text, parse_mode='HTML')

@bot.message_handler(
    func=lambda message: bool(message.text) and 'від 14 до 16 років' in message.text
)
def minor_14_16_algorithm_info(message):
    text = (
        "🎒 <b>Дії у разі правопорушення особою від 14 до 16 років:</b>\n\n"
        "1. <b>Обов'язково увімкнути боді-камеру.</b>\n"
        "2. <b>Встановити дані неповнолітнього</b> та викликати/повідомити його батьків (або законних представників).\n"
        "3. <b>Оскільки за ст. 184 КУпАП</b> або спеціальними нормами відповідальність за деякі правопорушення неповнолітніх віком від 14 до 16 років покладається на батьків або розглядається окремо:\n"
        "   • збираються докази та пояснення в присутності батьків, психолога чи педагога;\n"
        "   • складаються матеріали відносно батьків за невиконання обов'язків (або проф. матеріали залежно від статті);\n"
        "4. <b>Повідомити керівництво ЗЗСО та ювенальну превенцію.</b>\n"
        "5. <b>Оформити рапорт</b> та долучити повний пакет документів (свідоцтво про народження/паспорт дитини, паспорти батьків, пояснення)."
    )
    bot.send_message(message.chat.id, text, parse_mode='HTML')
...
def minor_under_14_algorithm_info(message):
    text = (
        "👶 <b>Дії у разі правопорушення особою до 14 років (малолітні):</b>\n\n"
        "1. <b>Обов'язково увімкнути боді-камеру.</b>\n"
        "2. <b>Встановити особу дитини</b> та невідкладно повідомити адміністрацію школи, психолога та батьків (законних представників).\n"
        "3. <b>Провести бесіду та опитування виключно в присутності психолога/педагога та батьків.</b>\n"
        "4. <b>Оскільки малолітні особи не підлягають адміністративній чи кримінальній відповідальності:</b>\n"
        "   • за необхідності оформлюються матеріали стосовно батьків за ст. 184 КУпАП (ухилення від виховання);\n"
        "   • інформуються служби у справах дітей та соціальні служби.\n"
        "5. <b>Скласти рапорт та зареєструвати подію.</b>"
    )
    bot.send_message(message.chat.id, text, parse_mode='HTML')
...
def handle_vnp_report_example(call):
    ...
    text = (
        ...
        "який повідомив, що помітив підозрілий предмет біля школи. Інспектором було здійснено додатковий "
        "обхід території та у клумбі навпроти входу до ЗЗСО було виявлено предмет за зовнішніми ознаками схожий на СВП. "
        "Про подію було проінформовано директора Харківського ліцею № 107 Єсауленко Світлану Володимирівнк. "
        "Інспектором СОБ було огороджено місце події та вжито усіх необхідних заходів задля забезпечення безпеки "
        "усіх учасників освітнього процесу. Про виявлення предмету було повідомлено ЧЧ УПП в Х/о ДПП, ЧЧ ХРУП 1 та сповіщено на лінію 102"
    )
...
if __name__ == '__main__':
    ...
    while True:
        try:
            ...
            print(f'Помилка під час роботи бота: {e}')
            time.sleep(5)
